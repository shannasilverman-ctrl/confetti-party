...
| E2E + axe | `CI=1 PW_REUSE=0 bun run test:e2e:desktop` then `CI=1 PW_REUSE=0 bun run test:e2e:mobile` (Playwright starts a fresh Wrangler server for each responsive matrix; config resolves through `scripts/wrangler-config-path.mjs` → `.output/server/wrangler.json` on CI / `dist/server/wrangler.json` in the Lovable sandbox) |
