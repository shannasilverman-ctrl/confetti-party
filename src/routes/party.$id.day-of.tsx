...
export const Route = createFileRoute("/_authenticated/party/$id/day-of")({
  component: DayOfPage,
});
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-2xl px-4 pb-24 pt-4">
        ...
        <section className="mt-4 rounded-3xl border border-border bg-card p-5 shadow-card">
          ...
          <h1 className="mt-1 font-display text-2xl font-semibold text-secondary">Next three actions</h1>
          <ul className="mt-3 space-y-2">
            {nextThree.length === 0 ? (
              <li className="rounded-xl bg-muted/40 p-3 text-sm text-muted-foreground">All set — you're floating.</li>
            ) : nextThree.map((t) => (
              <li key={t.id}>
                <button
          ...
          {(party.hostUpdates ?? []).length > 0 && (
            <ul className="mt-3 space-y-1.5">
              {(party.hostUpdates ?? []).slice(0, 3).map((u) => (
                <li key={u.id} className="rounded-lg bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
                  {u.text}
                </li>
              ))}
            </ul>
        ...
        <Card className="mt-4 p-5">
          <div className="mb-2 flex items-center justify-between text-sm font-semibold text-foreground">
            <span className="flex items-center gap-2"><UserCheck className="h-4 w-4 text-primary" /> Arrivals</span>
            <span className="text-xs text-muted-foreground">{arrived} / {yesGuests.length} in</span>
          </div>
          {yesGuests.length === 0 ? (
              ...
              {yesGuests.map((g) => {
                ...
                return (
                  <li key={g.id}>
                    ...
                    >
                      {here ? <CheckCircle2 className="h-5 w-5 text-primary" /> : <Circle className="h-5 w-5 text-muted-foreground" />}
